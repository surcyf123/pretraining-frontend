...
export async function fetchMetagraphData(): Promise<{
  ...
  const neuronData = json.neuron_data.map(
    ({
      ...
      neuronRank: each_neuron_ranks.toFixed(4),
      neuronIncentives: each_neuron_incentives.toFixed(4),
      neuronEmission: each_neuron_emission.toFixed(4),
      neuronConsensus: each_neuron_consensus.toFixed(4),
      neuronTrust: each_neuron_trust.toFixed(4),
      neuronValidatorTrust: each_neuron_validator_trust.toFixed(4),
      neuronDividends: each_neuron_dividends.toFixed(4),
      bonds: (each_neuron_bonds ?? 0).toFixed(4),
      neuronWeight: (each_neuron_weights ?? 0).toFixed(4),
      neuronStake: each_neuron_stake.toFixed(4),
